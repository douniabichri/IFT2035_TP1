...
pChar :: Char -> Parser () --`pChar :: Char -> Parser () est un parseur qui recherche un caractère spécifique dans l'entrée et ne renvoie rien.
pChar c = do { _ <- char c; return () }

-- Les commentaires commencent par un point-virgule et se terminent
-- à la fin de la ligne.
pComment :: Parser () --`pComment :: Parser () est un parseur pour les commentaires. Les commentaires commencent par un point-virgule ; et se terminent à la fin de la ligne. Ce parseur cherche un ;, puis consomme tous les caractères jusqu'à la fin de la ligne.
...
pQuote :: Parser Sexp --gère les expressions citées, qui commencent par un ', un ` ou une , et renvoie un sexp
...
-- Une liste (Tsil) est de la forme ( [e .] {e} ) --même chose pour les liste commencant par (
pTsil :: Parser Sexp
pTsil = do _ <- char '('
           pSpaces
           (do { _ <- char ')'; return Snil } --si la liste est vide retourne snil
            <|> do hd <- (do e <- pSexp
                             pSpaces
                             (do _ <- char '.'
                                 pSpaces
...
pSexpTop :: Parser Sexp -- Ce parseur tente d'analyser une expression, qui peut être une liste, une citation, un symbole ou un nombre.
...
-- générique "read".
instance Read Sexp where --Cette instance permet d'utiliser la fonction read sur une chaîne pour convertir cette chaîne en une Sexp.
...
data Ltype = Lint
           | Larw Ltype Ltype   -- Type "arrow" des fonctions = les types flèches
           deriving (Show, Eq)
